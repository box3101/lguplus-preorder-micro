export function initializeNotice() {
  const noticeMenu = document.querySelector(".common-notice-menu");
  ...
  function switchNoticeTab(targetNotice) {
    ...
    // 선택된 NOTIce 항목 활성화
    const activeNotice = noticeMenu.querySelector(
      `[data-tab="${targetNotice}"]`
    );
    if (activeNotice) {
